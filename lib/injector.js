...
var TRACE_ENTRY = 'var __njsEntryData__ = __njsTraceEntry__({file: %s, name: %s, line: %s, args: %s, argNames: %s});';
...
Injector.prototype.injectTracing = function(filename, code, wrapFunctions, includeArguments, wrappedFile) {
	...
	var output = falafel(code, {range: true, loc: true}, function(node) {
		...
		if (name) {
			...
			var argNames = node.params.map(function(p){return p.name});

			// put our TRACE_ENTRY as the first line of the function and TRACE_EXIT as last line
			var traceEntry = util.format(TRACE_ENTRY, JSON.stringify(filename), JSON.stringify(name), node.loc.start.line, args, JSON.stringify(argNames));
